...
export function localizedDistributionText(
  ...
  const normalizedEntries = entries.map(([locale, value]) => [
  ...
  for (const locale of locales) {
    const language = locale.trim().replaceAll('_', '-').toLowerCase().split('-')[0]
    const languageMatch = normalizedEntries.find(([key]) => key.split('-')[0] === language)
    if (languageMatch) return languageMatch[1]
  }

  return entries[0][1] || fallback
}
export async function fetchDistributionProducts(publisherName: string): Promise<DistributionProduct[]> {
  const response = await apiFetch(distributionPath(`/publishers/${encodeURIComponent(publisherName)}/products`))
  const payload = await safeJsonParse<{ data: DistributionProduct[] }>(response)
  return payload.data ?? []
}
...
export async function createDistributionProduct(
  ...
  const response = await apiFetch(
    distributionPath(`/publishers/${encodeURIComponent(publisherName)}/products`),
...
export async function fetchDistributionChannels(productId: string): Promise<DistributionChannel[]> {
  const response = await apiFetch(distributionPath(`/products/${encodeURIComponent(productId)}/channels`))
  const payload = await safeJsonParse<{ data: DistributionChannel[] }>(response)
  return payload.data ?? []
}

export async function fetchDistributionReleases(productId: string, channel: string): Promise<DistributionRelease[]> {
  const params = new URLSearchParams({ channel, limit: '100', offset: '0' })
  const response = await apiFetch(distributionPath(`/products/${encodeURIComponent(productId)}/releases?${params}`))
  const payload = await safeJsonParse<{ data: DistributionRelease[] }>(response)
  return payload.data ?? []
}
...
export async function updateDistributionChannel(
  ...
  const response = await apiFetch(
    distributionPath(`/products/${encodeURIComponent(productId)}/channels/${encodeURIComponent(channelId)}`),
    {
      method: 'PUT',
      body: JSON.stringify({
        display_name: input.displayName,
...
export async function prepareDistributionUpload(
  ...
  const response = await apiFetch(
    distributionPath(`/products/${encodeURIComponent(productId)}/artifacts/upload-url`),
    { method: 'POST', body: JSON.stringify({ file_name: input.fileName, mime_type: input.mimeType }) },
  )
  return safeJsonParse<DistributionUpload>(response)
}

export async function uploadDistributionArtifact(upload: DistributionUpload, file: File, mimeType: string, hash: string) {
  const response = await fetch(upload.uploadUrl, {
    method: 'PUT',
    headers: { 'Content-Type': mimeType || file.type || 'application/octet-stream', 'x-amz-meta-sha256': hash },
    body: file,
  })
...
export async function createDistributionRelease(
  ...
  input: {
    ...
    forceUpdate?: boolean
    artifacts?: Array<{ objectKey: string; platform: string; architecture: string }>
  },
...
export async function updateDistributionRelease(
  ...
  const response = await apiFetch(
    distributionPath(`/products/${encodeURIComponent(productId)}/releases/${encodeURIComponent(releaseId)}`),
    {
      method: 'PUT',
      body: JSON.stringify({
        version: input.version,
...
async function postDistributionUpdate(
  ...
  return {
    updateAvailable: Boolean(payload.update_available ?? payload.updateAvailable),
    currentVersion: String(payload.current_version ?? payload.currentVersion ?? input.version),
    release: (payload.release ?? payload.new_release ?? payload.newRelease ?? null) as DistributionRelease | null,
  }
}
...
export function checkDistributionProductUpdate(
  ...
  return postDistributionUpdate(`/api/products/${encodeURIComponent(productId)}/update/check`, input)
}
...
export function requestDistributionProductUpdate(
  ...
  return postDistributionUpdate(`/api/products/${encodeURIComponent(productId)}/update`, input)
}
...
export function checkDistributionAppUpdate(
  ...
  return postDistributionUpdate(`/api/apps/${encodeURIComponent(appId)}/update/check`, input)
}
...
export function requestDistributionAppUpdate(
  ...
  return postDistributionUpdate(`/api/apps/${encodeURIComponent(appId)}/update`, input)
}


export async function publishDistributionRelease(productId: string, releaseId: string): Promise<DistributionRelease> {
  const response = await apiFetch(
    distributionPath(`/products/${encodeURIComponent(productId)}/releases/${encodeURIComponent(releaseId)}/publish`),
    { method: 'POST' },
  )
  return safeJsonParse<DistributionRelease>(response)
}

export async function fetchDistributionMetrics(productId: string, from?: string, to?: string): Promise<DistributionMetrics> {
  const params = new URLSearchParams()
  if (from) params.set('from', from)
  if (to) params.set('to', to)
  const query = params.toString()
  const response = await apiFetch(
    distributionPath(`/products/${encodeURIComponent(productId)}/metrics${query ? `?${query}` : ''}`),
  )
